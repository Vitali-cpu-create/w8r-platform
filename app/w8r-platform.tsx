...
export default function W8RPlatform({ viewer }: { viewer: Viewer }) {
  ...
  return <main className="platform-shell">
    ...
    <aside className="rail">
      ...
      <div className="suite-mark"><Image src="/ofpay-symbol-final-vector.svg" alt="" width={29} height={29} /><div><b>PRIVATE DEMO</b><span>Investor release 0.2</span></div></div>
    ...
    <section className="workspace" id="workspace">
      <header className="topbar"><div><span>W8R / BTLR / OfPay</span><Badge tone="teal">INTERACTIVE PROOF</Badge></div><div className="viewer"><span>{viewer.authenticated ? "Private workspace" : "Local sandbox"}</span><b>{viewer.displayName}</b><i>{viewer.displayName.slice(0, 2).toUpperCase()}</i></div></header>
      ...
      {screen === "migration" && <div className="screen">
        ...
        <div className="split">
          <article className="panel"><div className="panel-title"><div><span>MIGRATION RUN</span><h2>Maison North / Shopify</h2></div><strong>{state.migrationProgress}%</strong></div><Progress value={state.migrationProgress} />
            <div className="task-list">{[["Catalogue & variants", 428, true], ["Media and digital files", 1692, state.migrationProgress > 35], ["Customer permissions", 18420, state.migrationProgress > 55], ["Orders and tax history", 54882, state.migrationProgress > 72], ["App dependency map", 37, state.migrationProgress === 100]].map(item => <div key={item[0]}><i className={item[2] ? "done" : ""}>{item[2] ? "✓" : "···"}</i><span><b>{item[0]}</b><small>{item[1].toLocaleString()} records</small></span><em>{item[2] ? "Verified" : "Queued"}</em></div>)}</div>
      ...
      {screen === "resale" && <div className="screen">
        ...
        <div className="resale-layout"><article className="listing-card"><div className="listing-visual"><Badge tone="teal">AUTHENTICATED</Badge><span>A88<br />042</span></div><div><span>ATELIER 88</span><h2>Nocturne Carryall</h2><p>Condition: Excellent · Original receipt · Brisbane, AU</p><div className="bid"><small>CURRENT BID</small><strong>A$242.00</strong><em>8 bids · 02:14:08</em></div></div></article>
          <article className="panel escrow"><div className="panel-title"><div><span>ESCROW ORCHESTRATION</span><h2>Ownership handover</h2></div><Badge tone="pink">PARTNER-GATED</Badge></div><div className="escrow-steps">{[["Listing signed", state.escrowStatus !== "not-listed"], ["Buyer funds secured", ["funded", "released"].includes(state.escrowStatus)], ["Tracked shipment", state.escrowStatus === "released"], ["Inspection window", state.escrowStatus === "released"], ["Receipt + funds released", state.escrowStatus === "released"]].map((item, index) => <div className={item[1] ? "complete" : ""} key={item[0]}><i>{item[1] ? "✓" : index + 1}</i><span>{item[0]}</span></div>)}</div>
